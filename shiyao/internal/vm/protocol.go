...
// Environment variables accepted from the host are deliberately limited to
// non-loader, non-shell-control variables. The guest agent's own environment
// and /etc/shiyao-env remain trusted image configuration.
...
func (r ExecRequest) Validate() error {
	...
	for key, value := range r.Env {
		...
		if !allowedEnvKey.MatchString(key) {
			return fmt.Errorf("environment variable %q is not permitted", key)
		}
