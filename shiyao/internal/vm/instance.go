...
func NewInstance(id, socketPath string, cfg Config, netCfg NetworkConfig, vsockCfg VsockConfig, snapCfg SnapshotConfig) *Instance {
...
func (i *Instance) validateSnapshotIntegrity() error {
	...
	if i.snapInteg.ExpectedMemHash != "" {
		...
		if err != nil { return fmt.Errorf("compute memory file hash: %w", err) }
		if memHash != i.snapInteg.ExpectedMemHash { return fmt.Errorf("memory file hash mismatch: expected %s, got %s", i.snapInteg.ExpectedMemHash, memHash) }
	}
	if i.snapInteg.ExpectedStateHash != "" {
		stateHash, err := computeFileHash(i.snapCfg.StateFilePath)
		if err != nil { return fmt.Errorf("compute state file hash: %w", err) }
		if stateHash != i.snapInteg.ExpectedStateHash { return fmt.Errorf("state file hash mismatch: expected %s, got %s", i.snapInteg.ExpectedStateHash, stateHash) }
	}
	return nil
}
...
// guestKernelArgs constructs the trusted kernel command line. A caller may not
// override init: the guest agent must remain PID 1 for the non-snapshot boot path.
...
func (i *Instance) Configure(ctx context.Context) error {
	...
	if err := SetupTAP(ctx, i.netCfg); err != nil { i.state = StateCreated; return fmt.Errorf("setup tap: %w", err) }
	i.cleanups = append(i.cleanups, func() error { return CleanupTAP(ctx, i.netCfg.TapName) })
	if err := SetupFirewall(ctx, i.netCfg); err != nil { i.state = StateCreated; return fmt.Errorf("setup firewall: %w", err) }
	...
	fcConfig := fc.Config{
		...
		NetworkInterfaces: []fc.NetworkInterface{{StaticConfiguration: &fc.StaticNetworkConfiguration{HostDevName: i.netCfg.TapName, MacAddress: macAddr, IPConfiguration: &fc.IPConfiguration{IPAddr: *guestNetwork, Gateway: gatewayIP}}}},
...
func (i *Instance) Start(ctx context.Context) error {
	...
	if i.snapCfg.EnableResume {
		...
		i.state = StateRunning; return nil
	}
	if err := i.machine.Start(ctx); err != nil { return fmt.Errorf("start guest os: %w", err) }
...
func (i *Instance) Stop(ctx context.Context) error {
	...
	for j := len(i.cleanups)-1; j >= 0; j-- {
		if err := i.cleanups[j](); err != nil { errs = append(errs, fmt.Errorf("cleanup step %d: %w", j, err)); remainingCleanups = append(remainingCleanups, i.cleanups[j]) }
	}
