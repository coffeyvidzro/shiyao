...
func TestExecRequestRejectsDangerousEnvironment(t *testing.T) {
	req := ExecRequest{Version: ProtocolVersion, ID: "1", Command: "/bin/true", Env: map[string]string{"LD_PRELOAD": "/tmp/evil.so"}}
	if err := req.Validate(); err == nil { t.Fatal("expected dangerous environment variable to be rejected") }
}

func TestExecRequestRejectsExcessiveTimeout(t *testing.T) {
	req := ExecRequest{Version: ProtocolVersion, ID: "1", Command: "/bin/true", TimeoutMS: MaxTimeoutMS + 1}
	if err := req.Validate(); err == nil { t.Fatal("expected excessive timeout to be rejected") }
}
