...
func SetupFirewall(ctx context.Context, cfg Config) error {
	...
	if err := ipt.Append("filter", chain, "-m", "conntrack", "--ctstate", "ESTABLISHED,RELATED", "-j", "ACCEPT"); err != nil {
		cleanup()
		return fmt.Errorf("allow established traffic: %w", err)
	}
	if err := ipt.Append("filter", chain, "-p", "udp", "-d", cfg.HostIP, "--dport", "53", "-j", "ACCEPT"); err != nil {
		cleanup()
		return fmt.Errorf("allow host UDP DNS: %w", err)
	}
	if err := ipt.Append("filter", chain, "-p", "tcp", "-d", cfg.HostIP, "--dport", fmt.Sprintf("%d", HostProxyPort), "-j", "ACCEPT"); err != nil {
		cleanup()
	...
	for _, port := range cfg.AllowedPorts {
		...
		if err := ipt.Append("filter", chain, "-p", "tcp", "-d", cfg.HostIP, "--dport", fmt.Sprintf("%d", port), "-j", "ACCEPT"); err != nil {
			cleanup()
	...
	if err := ipt.Append("filter", "FORWARD", "-o", cfg.TapName, "-m", "conntrack", "--ctstate", "ESTABLISHED,RELATED", "-j", "ACCEPT"); err != nil {
		cleanup()
	...
	if err := ip6t.NewChain("filter", v6Chain); err != nil {
		cleanup()
		return fmt.Errorf("create IPv6 chain %s: %w", v6Chain, err)
	}
	v6Cleanup := func() {
		_ = ip6t.Delete("filter", "FORWARD", "-i", cfg.TapName, "-j", v6Chain)
		_ = ip6t.ClearChain("filter", v6Chain)
		_ = ip6t.DeleteChain("filter", v6Chain)
	}
	if err := ip6t.Append("filter", v6Chain, "-j", "DROP"); err != nil {
		v6Cleanup()
		cleanup()
		return fmt.Errorf("add IPv6 default drop: %w", err)
	}
	if err := ip6t.Append("filter", "FORWARD", "-i", cfg.TapName, "-j", v6Chain); err != nil {
		v6Cleanup()
		cleanup()
		return fmt.Errorf("attach IPv6 guest rule: %w", err)
	}
	return nil
}
...
func CleanupFirewall(ctx context.Context, cfg Config) error {
	...
	if err := ipt.Delete("filter", "FORWARD", "-o", cfg.TapName, "-m", "conntrack", "--ctstate", "ESTABLISHED,RELATED", "-j", "ACCEPT"); err != nil && !isRuleNotFound(err) {
	...
	if err == nil {
		v6Chain := firewallIPv6ChainName(cfg.TapName)
		if err := ip6t.Delete("filter", "FORWARD", "-i", cfg.TapName, "-j", v6Chain); err != nil && !isRuleNotFound(err) {
			errs = append(errs, fmt.Sprintf("delete IPv6 hook: %v", err))
		}
		if err := ip6t.ClearChain("filter", v6Chain); err != nil && !isChainNotFound(err) {
			errs = append(errs, fmt.Sprintf("clear IPv6 chain %s: %v", v6Chain, err))
		}
		if err := ip6t.DeleteChain("filter", v6Chain); err != nil && !isChainNotFound(err) {
			errs = append(errs, fmt.Sprintf("delete IPv6 chain %s: %v", v6Chain, err))
		}
	} else {
		errs = append(errs, fmt.Sprintf("init ip6tables during cleanup: %v", err))
	}
	if len(errs) > 0 {
