...
func (h *Handler) List(c *gin.Context) {
	...
	for _, sess := range sessions { items = append(items, newResponse(sess)) }
...
func (h *Handler) Revoke(c *gin.Context) {
	...
	if err != nil { httputil.Error(c, apperrors.NewBadRequest("invalid session id")); return }

	if err := h.service.Revoke(c.Request.Context(), sessionID, userID); err != nil { httputil.Error(c, err); return }

	currentSessionID, err := h.sessionID(c)
	if err == nil && currentSessionID == sessionID { ClearCookie(c) }
...
func UserIDFromContext(c *gin.Context) (uuid.UUID, error) {
	...
	if !exists { return uuid.Nil, apperrors.NewUnauthorized("authentication required") }
	userID, ok := value.(uuid.UUID)
	if !ok { return uuid.Nil, apperrors.NewUnauthorized("invalid authentication context") }
	return userID, nil
}
...
func newResponse(sess sqlc.Session) Response {
	...
	if sess.IpAddress != nil { value := sess.IpAddress.String(); ipAddress = &value }
