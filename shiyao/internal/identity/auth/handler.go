...
func (h *Handler) LoginWithPassword(c *gin.Context) {
	...
	if err != nil { httputil.Error(c, apperrors.NewBadRequest("invalid transaction_id")); return }
...
func (h *Handler) SendOTP(c *gin.Context) {
	...
	transactionID, err := parseUUID(req.TransactionID)
	if err != nil { httputil.Error(c, apperrors.NewBadRequest("invalid transaction_id")); return }
	if _, err := h.service.SendOTP(c.Request.Context(), transactionID); err != nil { httputil.Error(c, err); return }
...
func (h *Handler) SetPassword(c *gin.Context) {
	...
	if !ok { httputil.Error(c, apperrors.NewUnauthorized("authentication required")); return }
