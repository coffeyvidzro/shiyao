...
func (h *Handler) List(c *gin.Context) {
	...
	if err != nil { httputil.Error(c, err); return }

	responses := make([]Response, 0, len(sandboxes))
	for _, item := range sandboxes { responses = append(responses, newResponse(item)) }
...
func (h *Handler) Get(c *gin.Context) {
	...
	if err != nil { httputil.Error(c, apperrors.NewBadRequest("invalid sandbox ID")); return }
...
func (h *Handler) Create(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil { httputil.Error(c, apperrors.NewBadRequest("invalid request body")); return }
...
func (h *Handler) Delete(c *gin.Context) {
	...
	if err := h.service.Delete(c.Request.Context(), userID, sandboxID); err != nil { httputil.Error(c, err); return }
...
func parseSandboxID(value string) (uuid.UUID, error) { return uuid.Parse(value) }

func newResponse(sandbox sqlc.Sandbox) Response {
	return Response{
		ID: sandbox.ID, UserID: sandbox.UserID, VMID: sandbox.VmId, Template: sandbox.Template,
		Status: sandbox.Status, VCPU: sandbox.Vcpu, MemoryMB: sandbox.MemoryMb,
		TimeoutSeconds: sandbox.TimeoutSeconds, AllowedHosts: sandbox.AllowedHosts,
		CreatedAt: helper.FormatTime(sandbox.CreatedAt), StartedAt: helper.FormatTime(sandbox.StartedAt),
		StoppedAt: helper.FormatTime(sandbox.StoppedAt), UpdatedAt: helper.FormatTime(sandbox.UpdatedAt),
	}
}
