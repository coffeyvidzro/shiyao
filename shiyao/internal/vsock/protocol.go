...
type Config struct { GuestCID uint32 }
...
func (r ExecRequest) Validate() error {
	...
	for key, value := range r.Env {
		if len(key) > MaxEnvKeyLen { return fmt.Errorf("environment key %q too long: %d > %d", key, len(key), MaxEnvKeyLen) }
		if !allowedEnvKey.MatchString(key) { return fmt.Errorf("environment variable %q is not permitted", key) }
		if len(value) > MaxEnvValLen { return fmt.Errorf("environment value for %q too long: %d > %d", key, len(value), MaxEnvValLen) }
	}
	return nil
}
