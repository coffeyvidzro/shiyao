...
import (
	...
	"github.com/shiyao/shiyao/internal/identity/users"
...
func (a *Auth) userEnabled(c *gin.Context, principal authn.Principal) bool {
	if a.users == nil || principal.Subject.Type != authn.SubjectUser || principal.Subject.ID == uuid.Nil { return false }
...
func setPrincipal(c *gin.Context, principal authn.Principal) {
	...
	if principal.Credential.Type == authn.CredentialSession { c.Set(session.ContextSessionID, principal.Credential.ID) }
}
