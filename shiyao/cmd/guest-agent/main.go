...
func run() error {
	...
	for {
		...
		if err != nil { return fmt.Errorf("accept guest vsock connection: %w", err) }
		go serve(conn)
	}
}
...
func serve(conn io.ReadWriteCloser) {
	...
	if err := decoder.Decode(&req); err != nil { sendError(conn, fmt.Errorf("decode request: %w", err)); return }
	result := execute(req)
...
func execute(req vm.ExecRequest) vm.ExecResult {
	result := vm.ExecResult{Version: vm.ProtocolVersion, ID: req.ID, ExitCode: -1}
	if err := req.Validate(); err != nil { result.Error = err.Error(); return result }
...
func loadPresetEnv() []string {
	...
	if err != nil { return env }
	for _, line := range strings.Split(string(data), "\n") {
		line = strings.TrimSpace(line)
		if line != "" && !strings.HasPrefix(line, "#") { env = append(env, line) }
	}
	return env
}

type limitedBuffer struct { limit int; buf []byte; truncated bool }
func (b *limitedBuffer) Write(p []byte) (int, error) {
	if b.truncated { return len(p), nil }
	remaining := b.limit - len(b.buf)
	if remaining <= 0 { b.truncated = true; return len(p), nil }
	if len(p) > remaining { b.buf = append(b.buf, p[:remaining]...); b.truncated = true; return len(p), nil }
